...
public class ExpGem : MonoBehaviour
{
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.CompareTag(Define.TAG_HERO))
            GiveExp(collision.GetComponent<Hero>(), true);
    }
    ...
    public void GiveExp(Hero targetHero, bool getExp)
    {
        _collider.enabled = false;
        _GiveExp(targetHero, getExp).Forget();
    }

    private async UniTaskVoid _GiveExp(Hero targetHero, bool getExp)
    {
